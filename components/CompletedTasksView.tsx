...
import { CheckCircle2, Trash2, Circle, Search, X, Filter, Archive, ArchiveRestore } from 'lucide-react';
...
interface CompletedTasksViewProps {
  ...
  onClose: () => void;
  onDeleteRegularTask?: (taskId: string) => void; // For tasks from main collection
}
...
const CompletedTasksView: React.FC<CompletedTasksViewProps> = ({
  ...
  const filteredTasks = tasks
    .filter(task => {
      // Only show tasks that are actually completed (either have completedAt or status DONE)
      const isCompletedTask = task.completedAt !== undefined || task.status === TaskStatus.DONE;
      ...
      // Date filter - use completedAt if available, otherwise fall back to dueDate or createdAt
      const taskCompletionDate = task.completedAt || task.dueDate || task.createdAt;
      const now = Date.now();
      ...
      if (filterDateRange === 'week') {
        matchesDate = taskCompletionDate >= oneWeekAgo;
      } else if (filterDateRange === 'month') {
        matchesDate = taskCompletionDate >= oneMonthAgo && taskCompletionDate < oneWeekAgo;
      } else if (filterDateRange === 'older') {
        matchesDate = taskCompletionDate < oneMonthAgo;
      }

      return matchesSearch && matchesProject && matchesAssignee && matchesPriority && matchesDate;
    })
    .sort((a, b) => (b.completedAt || b.dueDate || b.createdAt) - (a.completedAt || a.dueDate || a.createdAt));
  ...
  // Calculate metrics using completedAt if available, or status DONE
  const completedThisWeek = tasks.filter(task => {
    const isCompleted = task.completedAt !== undefined || task.status === TaskStatus.DONE;
    if (!isCompleted) return false;

    const taskDate = task.completedAt || task.dueDate || task.createdAt;
  ...
  const completedByProject = projects.map(project => {
    const count = tasks.filter(task => (task.completedAt !== undefined || task.status === TaskStatus.DONE) && task.projectId === project.id).length;
    return { projectId: project.id, projectName: project.name, count };
  });

  const completedByAssignee = TEAM_MEMBERS.map(member => {
    const count = tasks.filter(task => (task.completedAt !== undefined || task.status === TaskStatus.DONE) && task.assignee === member.name).length;
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-4xl h-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="border-b border-slate-800 p-4 flex items-center justify-between bg-slate-800/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Historial de Tareas Completadas</h2>
            <span className="bg-slate-700 text-xs px-2 py-1 rounded-full font-mono">
              {totalFiltered} / {totalCompleted} tareas
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b border-slate-800">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="bg-slate-800/40 rounded-lg p-3">
              <span className="text-2xl font-bold text-white block">{totalCompleted}</span>
              <span className="text-[10px] text-slate-500 uppercase font-bold">Total Completadas</span>
            </div>
            <div className="bg-emerald-500/10 rounded-lg p-3 border border-emerald-500/20">
              <span className="text-2xl font-bold text-emerald-400 block">{completedThisWeek}</span>
              <span className="text-[10px] text-emerald-400/70 uppercase font-bold">Esta Semana</span>
            </div>
        ...
        <div className="p-4 border-b border-slate-800">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="bg-purple-500/10 rounded-lg p-3 border border-purple-500/20">
              <span className="text-xl font-bold text-purple-400 block">{completedThisMonth}</span>
              <span className="text-[10px] text-purple-400/70 uppercase font-bold">Este Mes</span>
            </div>
            <div className="bg-orange-500/10 rounded-lg p-3 border border-orange-500/20">
              <span className="text-xl font-bold text-orange-400 block">{completedByProject.reduce((sum, item) => sum + item.count, 0)}</span>
            ...
            <div className="bg-pink-500/10 rounded-lg p-3 border border-pink-500/20">
              <span className="text-xl font-bold text-pink-400 block">{completedByAssignee.reduce((sum, item) => sum + item.count, 0)}</span>
        ...
        <div className="p-4 border-b border-slate-800 flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              value={searchQuery}
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${showFilters ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
            title="Filtros"
          >
            <Filter size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
          {filteredTasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-12">
              <Archive className="text-slate-600 mb-4" size={48} />
                              ...
                              <div className="flex items-center gap-2 ml-2">
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    } else if (onDeleteRegularTask) {
                                      onDeleteRegularTask(task.id);
                                    }
                                  }}
                                  className="text-red-400 hover:text-red-300 p-1 rounded hover:bg-slate-700 transition-colors"
                                  title={task.completedAt ? "Eliminar permanentemente" : "Eliminar tarea"}
                                >
                                  <Trash2 size={16} />
                                </button>
                              </div>
                            ...
                            <div className="flex items-center gap-3 text-xs">
                              ...
                              {(task.completedAt || task.status === TaskStatus.DONE) && (
                                <div className="flex items-center gap-1 text-slate-500">
                                  ...
                                  <span>Completada: {task.completedAt ? new Date(task.completedAt).toLocaleDateString() : new Date(task.dueDate || task.createdAt).toLocaleDateString()}</span>
                                </div>
                              )}
                              {task.dueDate && !task.completedAt && task.status !== TaskStatus.DONE && (
                                <div className="flex items-center gap-1 text-slate-500">
                                  <CheckCircle2 size={12} className="text-emerald-400" />
        ...
        <div className="p-4 border-t border-slate-800 flex items-center justify-between bg-slate-800/30">
          <p className="text-xs text-slate-500">
            Mostrando {totalFiltered} de {totalCompleted} tareas completadas
          </p>
          <button
            onClick={onClose}
